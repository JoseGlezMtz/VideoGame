...
app.post("/api/login", async (request, response) => {
  ...
  app.post("/api/registerGame", async(request, response)=>{
    const{Player_ID, Resultado} = request.body;

    let connection = null;

    try{
      connection = await connectToDB();
      await connection.execute("CALL Register_result(?, ?, @status_message);", [Player_ID, Resultado]);
